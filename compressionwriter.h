    ...
    public:
        explicit CompressionWriter(std::filesystem::path path, const lightdb::Codec &codec,
            ...
            : path_{std::move(path)},
              context_{0},
              lock_{context_},
              configuration_{Configuration{0, 0, 0, 0, 0, {framerate, 1}, {0, 0}}, codec.nvidiaId().value(), 0},
              gop_{0u},
              gop_size_{gop_size}
        { }
  ...
  //          assert(rx == cudaSuccess);
    ...
    private:
        ...
        void initialize_encoder(const size_t height, const size_t width) {
            configuration_ = {Configuration{static_cast<unsigned int>(width), static_cast<unsigned int>(height), 0, 0, 0, configuration_.framerate, {0, 0}}, configuration_.codec, static_cast<unsigned int>(gop_size_)};
